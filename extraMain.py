...
from src.model_poseformer import PoseTransformer,weight_init
import src.data_utils_new as data_utils
import src.CamerasNew as cameras 
import src.cameras as cameras_old
import src.utils as utils
import src.procrustes as procrustes
import src.viz_new as viz

os.environ["CUDA_VISIBLE_DEVICES"] = "0"
def get_all_batches(opt,data_x, data_y, rcams ,training=True ):
    ...
    # 2d pos 具有 16个关节点
    encoder_inputs  = np.zeros((n, 16*2+20), dtype=float)
    # 3d pose 
    ...
    for key2d in data_x.keys():
        ...
        tmp = rcams[key3d]
        tmp = np.resize(tmp,(n2d,4))

        data_tmp = np.concatenate((data_x[key2d],tmp),axis=1)

        parent = [1,2,4,5,7,8,9,10,11,12,13,14,15]
        child = [0,1,3,4,6,7,8,7,10,11,7,13,14]
        pp3d = np.reshape(data_y[key3d],(-1,16,3))
        dists = pp3d[ :,child, :] - pp3d[:,parent,:]
        dists_cat = np.concatenate((pp3d[:,0,:].reshape(-1,1,3),dists[:,0:2,:],pp3d[:,3,:].reshape(-1,1,3),dists[:,2:4,:],pp3d[:,6,:].reshape(-1,1,3),dists[:,4:,:]),axis=1)
        bone_length = np.linalg.norm(dists_cat,axis=2)
        data_tmp = np.concatenate((data_tmp,bone_length),axis=1)

        encoder_inputs[idx:idx+n2d, :]  = data_tmp
...
def main(opt):
    ...
    model = PoseTransformer(opt.predict_14)
    model = model.cuda()
    ...
    while current_epoch < opt.epochs:
        ...
        encoder_inputs, decoder_outputs = get_all_batches(opt, train_set_2d, train_set_3d, rcams, training=True )
        ...
        for i in range( nbatches ):
            ...
            in_temp = encoder_inputs[i]
            enc_in = torch.from_numpy(in_temp[:,0:32]).float()
            enc_in_extra = Variable(torch.from_numpy(in_temp[:,32:]).float()).cuda()
            if opt.predict_14:
              enc_in = enc_in.reshape(-1,14,2)
            else: 
              enc_in = enc_in.reshape(-1,16,2)
            ...
            outputs = model(inputs,enc_in_extra)

            # calculate loss
            optimizer.zero_grad()

            bone_length_true = enc_in_extra[:,4:]
            
            parent = [1,2,4,5,7,8,9,10,11,12,13,14,15]
            child = [0,1,3,4,6,7,8,7,10,11,7,13,14]
            pred3d = outputs.reshape(-1,16,3)
            dists = pred3d[ :,child, :] - pred3d[:,parent,:]
            dists_cat = torch.cat((pred3d[:,0,:].reshape(-1,1,3),dists[:,0:2,:],pred3d[:,3,:].reshape(-1,1,3),dists[:,2:4,:],pred3d[:,6,:].reshape(-1,1,3),dists[:,4:,:]),dim=1)
            pre_bone = torch.norm(dists_cat,dim=2)

            bone_loss = criterion(pre_bone, bone_length_true)
            loss_3d = criterion(outputs, targets)
            step_loss = 0.5*loss_3d + 0.5*bone_loss
            step_loss.backward() 
            ...
            tb_writer.add_scalar("step_loss", step_loss, glob_step)
            tb_writer.add_scalar("bone_loss", bone_loss, glob_step)
            tb_writer.add_scalar("loss_3d", loss_3d, glob_step)
        ...
        if opt.evaluateActionWise:
          ...
          for action in actions:
            ...
            encoder_inputs, decoder_outputs = get_all_batches(opt, action_test_set_2d, action_test_set_3d , rcams, training=False)
          ...
          tb_writer.add_scalar("avg_val", avg_val, glob_step)
...
def sample(opt):
  ...
  #rootPath = os.getcwd()
  rcams = cameras.load_cameras(opt.cameras_path, SUBJECT_IDS)
  rcams_old = cameras_old.load_cameras(opt.cameras_path, SUBJECT_IDS) 
  ...
  for key2d in test_set_2d.keys():
    ...
    parent = [1,2,4,5,7,8,9,10,11,12,13,14,15]
    child = [0,1,3,4,6,7,8,7,10,11,7,13,14]
    pp3d = np.reshape(test_set_3d[ key3d ],(-1,16,3))
    dists = pp3d[ :,child, :] - pp3d[:,parent,:]
    dists_cat = np.concatenate((pp3d[:,0,:].reshape(-1,1,3),dists[:,0:2,:],pp3d[:,3,:].reshape(-1,1,3),dists[:,2:4,:],pp3d[:,6,:].reshape(-1,1,3),dists[:,4:,:]),axis=1)
    bone_length = np.linalg.norm(dists_cat,axis=2)
    data_tmp = np.concatenate((tmp,bone_length),axis=1)
    ...
    data_tmp = np.array_split( data_tmp, n3d // opt.batch_size )
    ...
    for bidx in range( len(enc_in) ):
      ...
      ei = ei.reshape(-1,16,2)
      extra_ei = torch.from_numpy(data_tmp[bidx]).float()
      inputs = Variable(ei.cuda())
      input_extra = Variable(extra_ei.cuda())
      outputs = model(inputs,input_extra)
    ...
    if opt.camera_frame:
      ...
      scams = {(subj,c+1): rcams_old[(subj,c+1)] for c in range(N_CAMERAS)} # cams of this subject
